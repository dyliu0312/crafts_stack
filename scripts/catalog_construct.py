"""
A script to construct a galaxy catalog corresponding to the mock HIIM map. It select galaxies with the target frequency range and also satisfy given selection (e.g. Main Sample), both given by the flag dataset in the HDF5 file. It can optionally adding a frequency column and shifting coordinates.
"""
import os
import numpy as np
import pandas as pd
import h5py as h5
from mytools.constant import HI_restfreq
...
def construct_galcat_pd(
        ...
        add_freq_col=True, 
        shift=True, 
        curr_filed_center=[90,0],
        target_filed_center=[218, 42.5],
        freq0=HI_restfreq.to('MHz')
):
    """
    ...
    galcat_pd = pd.DataFrame(galcat, columns=cols)
    if add_freq_col:
        galcat_pd['Freq'] = galcat_pd['z'].map(lambda z: freq0 / (1 + z))

    if shift:
        print(f"Shift RA/DEC by {curr_filed_center} to {target_filed_center}")
        galcat_pd['RA'] = galcat_pd['RA'] - curr_filed_center[0] + target_filed_center[0]
        galcat_pd['DEC'] = galcat_pd['DEC'] - curr_filed_center[1] + target_filed_center[1]
    return galcat_pd

def select_gal(df, lim_ra=[180,255], lim_dec=[40,45], lim_freq=[1264,1307]):
    """
    Selects galaxies based on specified limits for RA, DEC, and frequency.
    """
    valid = (df['RA'] > lim_ra[0]) & (df['RA'] < lim_ra[1]) & \
            (df['DEC'] > lim_dec[0]) & (df['DEC'] < lim_dec[1]) & \
            (df['Freq'] > lim_freq[0]) & (df['Freq'] < lim_freq[1])
    return df[valid]
...
def _get_env_vars():
    ...
    """
    # NOTE: save_key is removed as it's not used in the corrected save logic.
    ...
    # Safely convert string values to booleans
    add_freq_col = os.environ.get('ADD_FREQ_COL', 'True').lower() in ('true', '1', 't')
    shift = os.environ.get('SHIFT', 'True').lower() in ('true', '1', 't')
    curr_filed_center = [float(x) for x in os.environ.get('CURR_FILED_CENTER', '90 0').split()]
    target_filed_center = [float(x) for x in os.environ.get('TARGET_FILED_CENTER', '218 42.5').split()]
    freq0 = float(os.environ.get('FREQ0', '1420.40575178'))
    ...
    args = dict(
        save_file=save_file,
        flag_file=flag_file, flag_key=flag_key, repeat=repeat,
        galcat_file=galcat_file, galcat_key=galcat_key, galcat_flag_key=galcat_flag_key,
        cols=cols,
        add_freq_col=add_freq_col, shift=shift,
        curr_filed_center=curr_filed_center, target_filed_center=target_filed_center,
        freq0=freq0,
        lim_ra=lim_ra, lim_dec=lim_dec, lim_freq=lim_freq,
    )
    return args
...
def main():
    ...
    flag = get_flag(args['flag_file'], args['flag_key'], args['repeat']) # type: ignore
    ...
    galcat_pd = construct_galcat_pd(
        ...
        args['galcat_key'], 
        args['cols'],
        args['add_freq_col'],  # type: ignore
        args['shift'],  # type: ignore
        args['curr_filed_center'], 
        args['target_filed_center'], 
        args['freq0']
    )
    ...
    galcat_pd_flaged_cut = select_gal(
        galcat_pd_flaged,
        args['lim_ra'], args['lim_dec'], args['lim_freq']
    )
