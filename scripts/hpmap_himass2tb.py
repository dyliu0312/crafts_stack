...
from astropy.constants import c, h, k_B, m_e, m_p  # type: ignore
...
A_HI = 2.86888e-15 / u_s
nu_HI = rest_freq * u_mhz
m_H = m_p + m_e


# ------------------ Functions ------------------
def compute_temp_channel(args):
    ...
    D_L = cosmo.luminosity_distance(z)  # type: ignore
    numerator = (1 + z) ** 3 * 3 * c ** 2 * h * A_HI
    denominator = 32 * np.pi * k_B * m_H * nu_HI * D_L ** 2 * omega_pix_sr * delta_nu
    tb_k = (numerator / denominator) * (mass_slice * input_mass_unit)
    return i, tb_k.to_value(output_temp_unit)


def convert_mass_cube_to_brightness_temp(mass_cube, freqs, nside,
                                         input_mass_unit=u_mass, output_temp_unit=u_mk, 
                                         nworker=None, delta_nu=None):
    ...
    tasks = [(i, mass_cube[i], z_channels[i], input_mass_unit, output_temp_unit,
              omega_pix_sr, delta_nu) for i in range(nfreq)]
    ...
    with Pool(processes=nworker) as pool:
        ...
        results = list(tqdm(pool.imap(compute_temp_channel, tasks), 
                           total=nfreq, desc="Converting mass to temperature"))
...
def apply_beam_smoothing(temp_cube, nside, pix_idx, beam_fwhm_arcmin, nworker=None):
    ...
    with Pool(processes=nworker) as pool:
        # Use tqdm to show progress
        results = list(tqdm(pool.imap(smooth_one_channel, tasks), 
                           total=nfreq, desc="Applying beam smoothing"))
...
if __name__ == "__main__":
    ...
    if not smooth_only:
        ...
        with h5py.File(input_masss_filename, "r") as f:
            mass_cube = f[key_mass][:]  # type: ignore
            pix_idx = f[key_pix][:]  # type: ignore
            freqs = f[key_freq][:]  # type: ignore
        ...
        temp_cube = convert_mass_cube_to_brightness_temp(
            mass_cube, freqs, nside, input_mass_unit, output_temp_unit, nworker, delta_nu # type: ignore
        )
        ...
        with h5py.File(output_temp_filename, "w") as fout:
            ...
            fout.create_dataset(key_t, data=temp_cube, compression="gzip")
        ...
        if beam_fwhm_arcmin is not None:
            ...
            final_cube = apply_beam_smoothing(temp_cube, nside, pix_idx, beam_fwhm_arcmin, nworker)

            with h5py.File(output_temp_filename, "a") as fout:
                fout.create_dataset(key_tc, data=final_cube, compression="gzip")
        ...
        if beam_fwhm_arcmin is None:
            raise ValueError("BEAM_FWHM_ARCMIN must be provided for smoothing only mode.")

        print(f"Loading temp cube from {output_temp_filename}...")
        with h5py.File(output_temp_filename, "r") as f:
            pix_idx = f[key_pix][:]  # type: ignore
            temp_cube = f[key_t][:]  # type: ignore
            nside = f.attrs[key_nside]
        print(f"Applying beam smoothing with FWHM = {beam_fwhm_arcmin} arcmin...")
        final_cube = apply_beam_smoothing(temp_cube, nside, pix_idx, beam_fwhm_arcmin, nworker)
