...
def plot_hp_grid(
    ...
    hp_kwargs : dict, optional
        Extra keyword arguments for the Healpy plotting function.
    """
    ...
    if n == 1:
        ...
        sub_title = title[0]
        if return_pmap:
            pm = hpfunc(data[0], hold=True, title=sub_title, return_projected_map=True,**hpfunc_kw)
            if graticule:
                hp.graticule(**kw_graticule)
            return axs, pm
        else:
            hpfunc(data[0], hold=True, title=sub_title, **hpfunc_kw)
            if graticule:
                hp.graticule(**kw_graticule)
            return axs
        ...
        for idx, ax in enumerate(axs):
            ...
            sub_title = title[idx]
            if return_pmap:
                pm = hpfunc(data[idx], hold=True, title=sub_title, return_projected_map=True, **hpfunc_kw)
                pm_list.append(pm)
                if graticule:
                    hp.graticule(**kw_graticule)
            else:
                hpfunc(data[idx], hold=True, title=sub_title, **hpfunc_kw)
                if graticule:
                    hp.graticule(**kw_graticule)
        if return_pmap:
            return axs, pm_list
